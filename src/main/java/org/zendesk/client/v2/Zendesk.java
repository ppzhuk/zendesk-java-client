...
public class Zendesk implements Closeable {
    ...
    private <T> ListenableFuture<T> submit(Request request, ZendeskAsyncCompletionHandler<T> handler) {
        ...
        try {
            if (curRemainingApiCalls != null && curRemainingApiCalls < minRemainingApiCalls) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Remaining API calls ({}) less then minimum value ({}). Fell asleep for 1 minute.",
    ...
    private void logResponse(Response response) throws IOException {
        if (logger.isDebugEnabled()) {
            logger.debug("Response HTTP/{} {} X-Rate-Limit: {}, X-Rate-Limit-Remaining: {}",
                    response.getStatusCode(),
                    response.getStatusText(),
                    response.getHeader("X-Rate-Limit"),
    ...
    private boolean isStatus2xx(Response response) {
        String header = response.getHeader("X-Rate-Limit-Remaining");
        if (header != null) {
            curRemainingApiCalls = Integer.parseInt(header);
        }
    ...
    public static class Builder {
        ...
        public Builder setReqInterval(Integer reqIntervalMs) {
            this.reqInterval = reqIntervalMs;
            return this;
        }
        ...
        public Zendesk build() {
            if (token != null) {
                return new Zendesk(client, url, username + "/token", token,
                        minRemainingApiCalls, reqInterval);
            } else if (oauthToken != null) {
                return new Zendesk(client, url, oauthToken, minRemainingApiCalls, reqInterval);
            }
            return new Zendesk(client, url, username, password, minRemainingApiCalls, reqInterval);
        }
    }
}
